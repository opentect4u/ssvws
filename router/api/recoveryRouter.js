...
recoveryRouter.post("/search_group_app", async (req, res) => {
    ...
    whr = `(a.group_code like '%${data.grp_dtls}%' OR b.group_name like '%${data.grp_dtls}%') AND c.status = 'A' AND c.tr_type = 'D'`,
    ...
    if(search_grp.suc > 0 && search_grp.msg.length > 0){
        var select = "a.loan_id,a.member_code,a.period,a.curr_roi,a.prn_disb_amt,a.intt_cal_amt,a.prn_amt,a.od_prn_amt,a.intt_amt,a.od_intt_amt,a.prn_emi,a.intt_emi,a.tot_emi,a.period,a.period_mode,a.instl_paid,a.instl_end_dt,b.client_name,c.status, c.balance, c.intt_balance, c.payment_date",
        ...
        AND a.group_code = ${search_grp.msg[0].group_code} AND c.status = 'A' AND c.tr_type = 'D'
