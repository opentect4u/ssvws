  ...
  loan_outstandingRouter.post("/loan_outstanding_report_groupwise", async (req, res) => {
            try {
                ...
                // }else {
                    ...
                    var select = "a.branch_code,e.branch_name,b.group_code,c.group_name,c.co_id,f.bank_name,c.acc_no1,c.acc_no2,b.period_mode,CASE WHEN b.period_mode = 'Monthly' THEN b.recovery_day WHEN b.period_mode IN ('Weekly','Fortnight') THEN CASE b.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,b.week_no,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding,d.emp_name co_name",
                    ...
                    order = `GROUP BY a.branch_code,e.branch_name,b.group_code,c.group_name,c.co_id,f.bank_name,c.acc_no1,c.acc_no2,b.period_mode,b.recovery_day,b.week_no,d.emp_name`;
                    var outstanding_data = await db_Select(select,table_name,whr,order);
...
//Outstanding report groupwise 13.10.2025 for currentdate/end of month
    ...
    loan_outstandingRouter.post("/loan_outstanding_report_groupwise_new", async (req, res) => {
            try {
                ...
                if (isCurrentDate) {
                    var select = "a.branch_code,d.branch_name,a.group_code,b.group_name,b.co_id,b.bank_name,b.acc_no1,b.acc_no2,a.period_mode,CASE WHEN a.period_mode = 'Monthly' THEN a.recovery_day WHEN a.period_mode IN ('Weekly','Fortnight') THEN CASE a.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,a.week_no,SUM(a.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt + a.od_prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.outstanding) outstanding,c.emp_name co_name",
                    table_name = "td_loan a LEFT JOIN md_group b ON a.group_code = b.group_code LEFT JOIN md_employee c ON b.co_id = c.emp_id LEFT JOIN md_branch d ON a.branch_code = d.branch_code",
                    // whr = `a.branch_code IN (${data.branch_code}) AND a.disb_dt <= '${data.supply_date}'`,
                    whr = `a.branch_code IN (${data.branch_code})`,
                    order = `GROUP BY a.branch_code,d.branch_name,a.group_code,b.group_name,b.co_id,b.bank_name,b.acc_no1,b.acc_no2,a.period_mode,a.recovery_day,a.week_no,c.emp_name
