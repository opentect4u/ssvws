    ...
    loan_outstandingRouter.post("/loan_outstanding_report_groupwise_new", async (req, res) => {
            try {
                ...
                if (isCurrentDate) {
                    ...
                    var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,d.emp_name co_name,(f.bank_name, ' - ',c.bank_name)AS bank_name,f.branch_name AS bank_branch,c.acc_no1,c.acc_no2,b.period_mode,CASE WHEN b.period_mode = 'Monthly' THEN b.recovery_day WHEN b.period_mode IN ('Weekly','Fortnight') THEN CASE b.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,b.week_no,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
