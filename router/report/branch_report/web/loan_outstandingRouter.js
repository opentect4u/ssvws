...
loan_outstandingRouter.post("/fetch_branch_name_based_usertype", async (req, res) => {
  ...
  //Outstanding report groupwise 13.03.2025 before 30th september
  ...
  loan_outstandingRouter.post("/loan_outstanding_report_groupwise", async (req, res) => {
            try {
                ...
                // }else {
                    ...
                    var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,f.bank_name,c.acc_no1,c.acc_no2,b.period_mode,CASE WHEN b.period_mode = 'Monthly' THEN b.recovery_day WHEN b.period_mode IN ('Weekly','Fortnight') THEN CASE b.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,b.week_no,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding,d.emp_name co_name",
                    table_name = "tt_loan_outstanding a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_group c ON b.group_code = c.group_code LEFT JOIN md_employee d ON c.co_id = d.emp_id LEFT JOIN md_branch e ON a.branch_code = e.branch_code LEFT JOIN md_bank f ON c.bank_name = f.bank_code",
                    whr = `a.balance_date = '${balance_date}' AND a.branch_code IN (${data.branch_code})`,
                    order = `GROUP BY a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,f.bank_name,c.acc_no1,c.acc_no2,b.period_mode,b.recovery_day,b.week_no,d.emp_name`;
                    var outstanding_data = await db_Select(select,table_name,whr,order);
     ...
     //Outstanding report fundwise 13.03.2025 before 30th september
     ...
     loan_outstandingRouter.post("/loan_outstanding_report_fundwise", async (req, res) => {
        try {
            ...
            // }else {
                ...
                var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,d.group_name,b.fund_id,c.fund_name,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                ...
                order = `GROUP BY a.branch_code,e.branch_name,e.area_code,b.group_code,d.group_name,b.fund_id,c.fund_name`;
                var outstanding_fund_data = await db_Select(select,table_name,whr,order);
    ...
    //Outstanding report cowise 13.03.2025 before 30th september
        ...
        try {
                ...
                // }else {
                    ...
                    if(res_dt.suc > 0 && res_dt.msg.length > 0){
                        ...
                        var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding,d.emp_name co_name",
                        table_name = "tt_loan_outstanding a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_group c ON b.group_code = c.group_code LEFT JOIN md_employee d ON c.co_id = d.emp_id LEFT JOIN md_branch e ON a.branch_code = e.branch_code",
                        whr = `a.balance_date = '${balance_date}' AND a.branch_code IN (${data.branch_code}) AND c.co_id IN (${data.co_id})`,
                        order = `GROUP BY a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,d.emp_name`;
                    var outstanding_co_data = await db_Select(select,table_name,whr,order);
    ...
    //Outstanding report branchwise 13.03.2025 before 30th september
        ...
        try {
                ...
                // }else {
                    ...
                    if(res_dt.suc > 0 && res_dt.msg.length > 0){
                        ...
                        var select = "a.branch_code,c.branch_name,c.area_code,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                        table_name = "tt_loan_outstanding a LEFT JOIN td_loan b ON a.branch_code = b.branch_code AND a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code",
                        whr = `a.balance_date = '${balance_date}' AND a.branch_code IN (${data.branch_code})`,
    ...
    //Outstanding report memberwise 17.03.2025 before 30th september    
        ...
        try {
            ...
            // }else {
                ...
                if(res_dt.suc > 0 && res_dt.msg.length > 0){
                    ...
                    var select = "a.branch_code,j.branch_name,j.area_code,a.loan_id,d.form_no,c.member_code,c.client_name,c.client_mobile,c.dob, TIMESTAMPDIFF(YEAR, c.dob, CURDATE()) AS age,c.gurd_name,c.husband_name,c.client_addr,c.voter_id,c.pan_no,c.aadhar_no,c.nominee_name,e.group_code,e.group_name,e.co_id,k.bank_name,e.acc_no1,e.acc_no2,f.emp_name co_name,b.fund_id,g.fund_name,b.scheme_id,h.scheme_name,b.purpose,CONCAT(i.sub_purpose,'-',i.purpose_id)purpose_id,b.applied_dt,b.applied_amt,b.disb_dt loan_date,b.prn_disb_amt loan_amount,b.curr_roi,b.period_mode,CASE WHEN b.period_mode = 'Monthly' THEN b.recovery_day WHEN b.period_mode IN ('Weekly','Fortnight') THEN CASE b.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,b.week_no,a.prn_amt prn_outstanding,a.intt_amt intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                    table_name = "tt_loan_outstanding a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_member c ON b.member_code = c.member_code LEFT JOIN td_grt_basic d ON b.member_code = d.member_code LEFT JOIN md_group e ON b.group_code = e.group_code LEFT JOIN md_employee f ON e.co_id = f.emp_id LEFT JOIN md_fund g ON b.fund_id = g.fund_id LEFT JOIN md_scheme h ON b.scheme_id = h.scheme_id LEFT JOIN md_purpose i ON b.purpose = i.purp_id LEFT JOIN md_branch j ON a.branch_code = j.branch_code LEFT JOIN md_bank k ON e.bank_name = k.bank_code",
                    whr = `a.balance_date = '${balance_date}' AND a.branch_code IN (${data.branch_code})`,
                    order = `GROUP BY a.branch_code,j.branch_name,j.area_code,a.loan_id,d.form_no,c.member_code,c.client_name,c.client_mobile,c.dob, TIMESTAMPDIFF(YEAR, c.dob, CURDATE()),c.gurd_name,c.husband_name,c.client_addr,c.voter_id,c.pan_no,c.aadhar_no,c.nominee_name,e.group_code,e.group_name,e.co_id,k.bank_name,e.acc_no1,e.acc_no2,f.emp_name,b.fund_id,g.fund_name,b.scheme_id,h.scheme_name,b.purpose,i.purpose_id,b.applied_dt,b.applied_amt,b.disb_dt,b.prn_disb_amt,b.curr_roi,b.period_mode,b.recovery_day,b.week_no,a.prn_amt,a.intt_amt
...
//Outstanding report groupwise 13.10.2025 for currentdate/end of month

    loan_outstandingRouter.post("/loan_outstanding_report_groupwise_new", async (req, res) => {
            try {
                var data = req.body;
                // console.log(data,'datas');
                ...
                const currentDate = new Date();
                const supplyDate = new Date(data.supply_date);
                // console.log(currentDate,supplyDate,'date');
                

                 // ✅ Validate input
                if (!data.branch_code || !data.supply_date) {
                    return res.send({
                        suc: 2,
                ...
                if (isCurrentDate) {
                    var select = "a.branch_code,d.branch_name,d.area_code,a.group_code,b.group_name,b.co_id,c.emp_name co_name,CONCAT(e.bank_name, ' - ',b.bank_name)AS bank_name,e.branch_name AS bank_branch,b.acc_no1,b.acc_no2,a.period_mode,CASE WHEN a.period_mode = 'Monthly' THEN a.recovery_day WHEN a.period_mode IN ('Weekly','Fortnight') THEN CASE a.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,a.week_no,SUM(a.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt + a.od_prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.outstanding) outstanding",
                    table_name = "td_loan a LEFT JOIN md_group b ON a.group_code = b.group_code LEFT JOIN md_employee c ON b.co_id = c.emp_id LEFT JOIN md_branch d ON a.branch_code = d.branch_code LEFT JOIN md_bank e ON b.bank_name = e.bank_code",
                    // whr = `a.branch_code IN (${data.branch_code}) AND a.disb_dt <= '${data.supply_date}'`,
                    whr = `a.branch_code IN (${data.branch_code})`,
                    order = `GROUP BY a.branch_code,d.branch_name,d.area_code,a.group_code,b.group_name,b.co_id,b.bank_name,e.bank_name,e.branch_name,b.acc_no1,b.acc_no2,a.period_mode,a.recovery_day,a.week_no,c.emp_name
                    HAVING SUM(a.outstanding) > 0
                    ...
                    var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,d.emp_name co_name,CONCAT(f.bank_name, ' - ',c.bank_name)AS bank_name,f.branch_name AS bank_branch,c.acc_no1,c.acc_no2,b.period_mode,CASE WHEN b.period_mode = 'Monthly' THEN b.recovery_day WHEN b.period_mode IN ('Weekly','Fortnight') THEN CASE b.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,b.week_no,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                    ...
                    order = `GROUP BY a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,c.bank_name,f.bank_name,f.branch_name,c.acc_no1,c.acc_no2,b.period_mode,b.recovery_day,b.week_no,d.emp_name
     ...
     //Outstanding report fundwise 13.10.2025 for currentdate/end of month
     
     loan_outstandingRouter.post("/loan_outstanding_report_fundwise_new", async (req, res) => {
        try {
            var data = req.body;
            // console.log(data,'datas');

             // ✅ Validate input
                if (!data.branch_code || !data.supply_date ||!data.fund_id) {
                    return res.send({
                        suc: 2,
                        msg: "Please provide valid input"
                    });
            ...
            // Choose table based on date
            if (isCurrentDate) {
                var select = "a.branch_code,d.branch_name,d.area_code,a.group_code,c.group_name,a.fund_id,b.fund_name,c.co_id,e.emp_name As co_name,CONCAT(f.bank_name, ' - ',c.bank_name) AS bank_name,f.branch_name AS bank_branch,SUM(a.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt + a.od_prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.outstanding) outstanding",
                table_name = "td_loan a LEFT JOIN md_fund b ON a.fund_id = b.fund_id LEFT JOIN md_group c ON a.group_code = c.group_code LEFT JOIN md_branch d ON a.branch_code = d.branch_code LEFT JOIN md_employee e ON c.co_id = e.emp_id LEFT JOIN md_bank f ON c.bank_name = f.bank_code",
                whr = `a.branch_code IN (${data.branch_code}) AND a.fund_id IN (${data.fund_id})`,
                order = `GROUP BY a.branch_code,d.branch_name,d.area_code,a.group_code,c.group_name,a.fund_id,b.fund_name,c.co_id,e.emp_name,c.bank_name,f.bank_name,f.branch_name
                HAVING SUM(a.outstanding) > 0
                ORDER BY a.branch_code,a.group_code`;
                var outstanding_fund_data = await db_Select(select,table_name,whr,order);
                const formattedDate = dateFormat(supplyDate, "yyyy-mm-dd");
                ...
                var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,d.group_name,b.fund_id,c.fund_name,d.co_id,f.emp_name As co_name,CONCAT(g.bank_name, ' - ',d.bank_name)AS bank_name,g.branch_name AS bank_branch,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                table_name = "td_loan_month_balance a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_fund c ON b.fund_id = c.fund_id LEFT JOIN md_group d ON b.group_code = d.group_code LEFT JOIN md_branch e ON a.branch_code = e.branch_code LEFT JOIN md_employee f ON d.co_id = f.emp_id LEFT JOIN md_bank g ON d.bank_name = g.bank_code",
                whr = `a.balance_date = '${dateFormat(supplyDate, "yyyy-mm-dd")}' AND a.branch_code IN (${data.branch_code}) AND b.fund_id IN (${data.fund_id})`,
                order = `GROUP BY a.branch_code,e.branch_name,e.area_code,b.group_code,d.group_name,b.fund_id,c.fund_name,d.co_id,f.emp_name,d.bank_name,g.bank_name,g.branch_name
                ORDER BY a.branch_code,b.group_code`;
                var outstanding_fund_data = await db_Select(select,table_name,whr,order);
                // outstanding_fund_data['balance_date'] = balance_date
    ...
    //Outstanding report cowise 13.10.2025 for currentdate/end of month
     
    loan_outstandingRouter.post("/loan_outstanding_report_cowise_new", async (req, res) => {
        try {
                var data = req.body;
                // console.log(data,'datas_co');
             ...
             // ✅ Validate input
                if (!data.branch_code || !data.supply_date ||!data.co_id) {
                    return res.send({
                        suc: 2,
                        msg: "Please provide valid input"
                    });
                ...
                // Choose table based on date
                if (isCurrentDate) {
                    var select = "a.branch_code,d.branch_name,d.area_code,a.group_code,b.group_name,b.co_id,c.emp_name co_name,CONCAT(e.bank_name,' - ',b.bank_name) AS bank_name,e.branch_name AS bank_branch,SUM(a.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt + a.od_prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.outstanding) outstanding",
                    table_name = "td_loan a LEFT JOIN md_group b ON a.group_code = b.group_code LEFT JOIN md_employee c ON b.co_id = c.emp_id LEFT JOIN md_branch d ON a.branch_code = d.branch_code LEFT JOIN md_bank e ON b.bank_name = e.bank_code",
                    whr = `a.branch_code IN (${data.branch_code}) AND b.co_id IN (${data.co_id})`,
                    order = `GROUP BY a.branch_code,d.branch_name,d.area_code,a.group_code,b.group_name,b.co_id,c.emp_name,b.bank_name,e.bank_name,e.branch_name
                    HAVING SUM(a.outstanding) > 0
                    ORDER BY a.branch_code,a.group_code`;
                    var outstanding_co_data = await db_Select(select,table_name,whr,order);
                    const formattedDate = dateFormat(supplyDate, "yyyy-mm-dd");
                    ...
                    //     var balance_date = dateFormat(res_dt.msg[0].balance_date, 'yyyy-mm-dd');
                        ...
                        var select = "a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,d.emp_name co_name,CONCAT(f.bank_name, ' - ',c.bank_name) AS bank_name,f.branch_name,SUM(b.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                        table_name = "td_loan_month_balance a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_group c ON b.group_code = c.group_code LEFT JOIN md_employee d ON c.co_id = d.emp_id LEFT JOIN md_branch e ON a.branch_code = e.branch_code LEFT JOIN md_bank f ON c.bank_name = f.bank_code",
                        whr = `a.balance_date = '${dateFormat(supplyDate, "yyyy-mm-dd")}' AND a.branch_code IN (${data.branch_code}) AND c.co_id IN (${data.co_id})`,
                        order = `GROUP BY a.branch_code,e.branch_name,e.area_code,b.group_code,c.group_name,c.co_id,d.emp_name,c.bank_name,f.bank_name,f.branch_name
                        ORDER BY a.branch_code,b.group_code`;
                    var outstanding_co_data = await db_Select(select,table_name,whr,order);
                    // outstanding_co_data['balance_date'] = balance_date
    ...
    //Outstanding report branchwise 13.10.2025 for currentdate/end of month
    
    loan_outstandingRouter.post("/loan_outstanding_report_branchwise_new", async (req, res) => {
        try {
                var data = req.body;
                // console.log(data,'datas_brn');
                ...
                // Choose table based on date
                if (isCurrentDate) {
                    var select = "a.branch_code,b.branch_name,b.area_code,SUM(a.prn_disb_amt) prn_disb_amt,SUM(a.prn_amt + a.od_prn_amt) prn_outstanding,SUM(a.intt_amt) intt_outstanding,SUM(a.outstanding) outstanding",
                    table_name = "td_loan a LEFT JOIN md_branch b ON a.branch_code = b.branch_code",
                    whr = `a.branch_code IN (${data.branch_code})`,
                    order = `GROUP BY a.branch_code,b.branch_name,b.area_code
                    HAVING SUM(a.outstanding) > 0
                    ORDER BY a.branch_code`;
                    var outstanding_branch_data = await db_Select(select,table_name,whr,order);
                    const formattedDate = dateFormat(supplyDate, "yyyy-mm-dd");
    ...
    //Outstanding report memberwise 13.10.2025 for currentdate/end of month    

    loan_outstandingRouter.post("/loan_outstanding_report_memberwise_new", async (req, res) => {
        try {
            var data = req.body;
            // console.log(data,'datas_member');
            ...
            // Choose table based on date
            if (isCurrentDate) {
                var select = "a.branch_code,i.branch_name,i.area_code,a.loan_id,c.form_no,a.member_code,b.client_name,b.client_mobile,TIMESTAMPDIFF(YEAR, b.dob, CURDATE()) AS age,b.dob,b.gurd_name,b.husband_name,b.client_addr,b.voter_id,b.pan_no,b.aadhar_no,b.nominee_name,d.group_code,d.group_name,d.co_id,e.emp_name co_name,CONCAT(j.bank_name,' - ',d.bank_name)AS bank_name,j.branch_name AS bank_branch,d.acc_no1,d.acc_no2,a.fund_id,f.fund_name,a.scheme_id,g.scheme_name,a.purpose,h.purpose_id,a.applied_dt,a.applied_amt,a.disb_dt loan_date,a.prn_disb_amt loan_amount,a.curr_roi,a.period_mode,CASE WHEN a.period_mode = 'Monthly' THEN a.recovery_day WHEN a.period_mode IN ('Weekly','Fortnight') THEN CASE a.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,a.week_no,(a.prn_amt + a.od_prn_amt) prn_outstanding,a.intt_amt intt_outstanding,a.outstanding outstanding",
                table_name = "td_loan a LEFT JOIN md_member b ON a.member_code = b.member_code LEFT JOIN td_grt_basic c ON b.member_code = c.member_code LEFT JOIN md_group d ON a.group_code = d.group_code LEFT JOIN md_employee e ON d.co_id = e.emp_id LEFT JOIN md_fund f ON a.fund_id = f.fund_id LEFT JOIN md_scheme g ON a.scheme_id = g.scheme_id LEFT JOIN md_purpose h ON a.purpose = h.purp_id LEFT JOIN md_branch i ON a.branch_code = i.branch_code LEFT JOIN md_bank j ON d.bank_name = j.bank_code",
                whr = `a.branch_code IN (${data.branch_code}) AND a.outstanding > 0`,
                order = `ORDER BY a.branch_code,i.branch_name,i.area_code,a.loan_id,a.member_code desc`;
                var outstanding_member_data = await db_Select(select,table_name,whr,order);
                const formattedDate = dateFormat(supplyDate, "yyyy-mm-dd");
                var balance_date = formattedDate;
                res.send({outstanding_member_data,balance_date});
                ...
                //     var balance_date = dateFormat(res_dt.msg[0].balance_date, 'yyyy-mm-dd');
                    ...
                    var select = "a.branch_code,j.branch_name,j.area_code,a.loan_id,d.form_no,c.member_code,c.client_name,c.client_mobile,c.dob, TIMESTAMPDIFF(YEAR, c.dob, CURDATE()) AS age,c.gurd_name,c.husband_name,c.client_addr,c.voter_id,c.pan_no,c.aadhar_no,c.nominee_name,e.group_code,e.group_name,e.co_id,f.emp_name co_name,CONCAT(k.bank_name,' - ',e.bank_name) AS bank_name,k.branch_name AS bank_branch,e.acc_no1,e.acc_no2,b.fund_id,g.fund_name,b.scheme_id,h.scheme_name,b.purpose,CONCAT(i.sub_purpose,'-',i.purpose_id)purpose_id,b.applied_dt,b.applied_amt,b.disb_dt loan_date,b.prn_disb_amt loan_amount,b.curr_roi,b.period_mode,CASE WHEN b.period_mode = 'Monthly' THEN b.recovery_day WHEN b.period_mode IN ('Weekly','Fortnight') THEN CASE b.recovery_day WHEN 1 THEN 'Sunday' WHEN 2 THEN 'Monday' WHEN 3 THEN 'Tuesday' WHEN 4 THEN 'Wednesday' WHEN 5 THEN 'Thursday'WHEN 6 THEN 'Friday'WHEN 7 THEN 'Saturday' ELSE 'Unknown' END ELSE 'N/A' END AS recovery_day,b.week_no,a.prn_amt prn_outstanding,a.intt_amt intt_outstanding,SUM(a.prn_amt + a.intt_amt) outstanding",
                    table_name = "td_loan_month_balance a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_member c ON b.member_code = c.member_code LEFT JOIN td_grt_basic d ON b.member_code = d.member_code LEFT JOIN md_group e ON b.group_code = e.group_code LEFT JOIN md_employee f ON e.co_id = f.emp_id LEFT JOIN md_fund g ON b.fund_id = g.fund_id LEFT JOIN md_scheme h ON b.scheme_id = h.scheme_id LEFT JOIN md_purpose i ON b.purpose = i.purp_id LEFT JOIN md_branch j ON a.branch_code = j.branch_code LEFT JOIN md_bank k ON e.bank_name = k.bank_code",
                    whr = `a.balance_date = '${dateFormat(supplyDate, "yyyy-mm-dd")}' AND a.branch_code IN (${data.branch_code})`,
                    order = `GROUP BY a.branch_code,j.branch_name,j.area_code,a.loan_id,d.form_no,c.member_code,c.client_name,c.client_mobile,c.dob, TIMESTAMPDIFF(YEAR, c.dob, CURDATE()),c.gurd_name,c.husband_name,c.client_addr,c.voter_id,c.pan_no,c.aadhar_no,c.nominee_name,e.group_code,e.group_name,e.co_id,k.bank_name,e.bank_name,k.branch_name,e.acc_no1,e.acc_no2,f.emp_name,b.fund_id,g.fund_name,b.scheme_id,h.scheme_name,b.purpose,i.purpose_id,b.applied_dt,b.applied_amt,b.disb_dt,b.prn_disb_amt,b.curr_roi,b.period_mode,b.recovery_day,b.week_no,a.prn_amt,a.intt_amt
