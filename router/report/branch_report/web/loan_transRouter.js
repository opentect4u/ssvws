...
loan_transRouter.post("/recov_loan_trans_report", async (req, res) => {
    var data = req.body;
      ...
      table_name = "td_loan_transactions a JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_group c ON b.group_code = c.group_code LEFT JOIN md_member d ON b.member_code = d.member_code LEFT JOIN md_scheme e ON b.scheme_id = e.scheme_id LEFT JOIN md_employee f ON a.created_by = f.emp_id",
      ...
      res.send(member_dt);
        ...
        var select = "a.payment_date,b.group_code,c.group_name,a.particulars,sum(a.credit)credit,(sum(a.balance) + sum(a.od_balance) + sum(a.intt_balance))balance,a.tr_mode,e.scheme_name,a.bank_name,a.created_by collector_code,f.emp_name collec_name,a.created_at,a.approved_by",
        table_name = "td_loan_transactions a JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_group c ON b.group_code = c.group_code LEFT JOIN md_member d ON b.member_code = d.member_code LEFT JOIN md_scheme e ON b.scheme_id = e.scheme_id LEFT JOIN md_employee f ON a.created_by = f.emp_id",
        whr = `b.branch_code = '${data.branch_code}' AND date(a.payment_date) BETWEEN '${data.from_dt}' AND '${data.to_dt}' AND a.tr_type = '${data.tr_type}'`,
        order = `GROUP BY a.payment_date,b.group_code,c.group_name,a.particulars,a.tr_mode,e.scheme_name,a.bank_name,a.created_by,f.emp_name,a.created_at,a.approved_by
