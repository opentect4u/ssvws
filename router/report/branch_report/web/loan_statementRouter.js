...
loan_statementRouter.post("/loan_statement_group_report", async (req, res) => {
    ...
    var select = "trans_date,sum(debit)debit,sum(credit)credit,sum(total)total_outstanding,IF(tr_mode = 'C', 'Cash', 'Bank Transfer')tr_mode,tr_type,particulars,IF(status = 'A', 'Approved', 'Unapproved')status",
    table_name = `(
        select a.payment_date trans_date,a.payment_id trans_id,a.tr_type,a.tr_mode,a.particulars particulars,SUM(a.debit) debit,SUM(a.credit) credit,SUM(a.balance + a.od_balance +a.intt_balance) total,a.status
        from td_loan_transactions a,td_loan b  
         where a.branch_id = '${data.branch_code}' AND a.loan_id = b.loan_id AND b.group_code = '${data.group_code}' AND date(a.payment_date) BETWEEN '${data.from_dt}' AND '${data.to_dt}' AND a.tr_type != 'O' AND a.tr_type != 'I'
         GROUP BY a.payment_date,a.payment_id,a.particulars,a.status,a.tr_type)a`,
    whr = null,
    order = `GROUP BY trans_date,tr_mode,tr_type,particulars,status
