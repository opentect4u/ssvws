...
attendancewebRouter.post("/fetch_absent_list", async (req, res) => {
    try{
        ...
        for(let dt of data.absent_data){
            var select = `(d.entry_dt,'yyyy-mm-dd')entry_dt,a.emp_id,b.emp_name,b.branch_id,c.branch_name`,
            table_name = `md_user a LEFT JOIN md_employee b ON a.emp_id = b.emp_id LEFT JOIN md_branch c ON b.branch_id = c.branch_code LEFT JOIN td_emp_attendance d ON a.emp_id = d.emp_id`,
        whr = `a.user_status = 'A' 
               AND a.emp_id <> 9999
               AND a.emp_id NOT IN (
                                  SELECT emp_id
