...
//    console.log(dateResult,first_dateResult, 'dmy');
  ...
  //create first date and last date
   ...
   var select = "a.branch_code,c.branch_name,b.group_code,d.group_name,d.co_id,e.emp_name co_name,MAX(b.disb_dt)disb_dt,SUM(b.prn_disb_amt)disb_amt,b.curr_roi,b.period,b.period_mode,b.instl_start_dt,b.instl_end_dt,SUM(b.tot_emi)tot_emi,SUM(a.dmd_amt)demand_amt,SUM(f.credit)collection_amt,(SUM(a.dmd_amt) - SUM(f.credit)) due_amt,SUM(b.outstanding)curr_outstanding",
...
dmd_vs_collRouter.post("/dmd_vs_collec_report_fundwise", async (req, res) => {
 ...
 `
   ...
   //create first date and last date
    ...
    var select = "a.branch_code,c.branch_name,b.group_code,d.group_name,d.co_id,e.emp_name co_name,b.fund_id,f.fund_name,SUM(b.tot_emi)total_emi,SUM(a.dmd_amt)demand_amt,SUM(g.credit)collection_amt,(SUM(a.dmd_amt) - SUM(g.credit)) due_amt,SUM(b.outstanding)curr_outstanding",
...
dmd_vs_collRouter.post("/dmd_vs_collec_report_cowise", async (req, res) => {
 ...
 `
   ...
   //create first date and last date
    ...
    var select = "a.branch_code,c.branch_name,b.group_code,d.group_name,d.co_id,e.emp_name co_name,SUM(b.tot_emi) total_emi,SUM(a.dmd_amt) demand_amt,SUM(f.credit)collection_amt,(SUM(a.dmd_amt) - SUM(f.credit)) due_amt,SUM(b.outstanding) curr_outstanding",
    table_name = "td_loan_month_demand a LEFT JOIN td_loan b ON a.branch_code = b.branch_code AND a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code LEFT JOIN md_group d ON b.group_code = d.group_code LEFT JOIN md_employee e ON d.co_id = e.emp_id LEFT JOIN td_loan_transactions f ON a.loan_id = f.loan_id",
...
dmd_vs_collRouter.post("/dmd_vs_collec_report_memberwise", async (req, res) => {
 ...
 `
   ...
   //create first date and last date
    ...
    var select = "a.branch_code,c.branch_name,b.loan_id,b.member_code,f.client_name,b.group_code,d.group_name,d.co_id,e.emp_name co_name,b.disb_dt,b.prn_disb_amt disb_amt,b.curr_roi,b.period,b.period_mode,b.instl_start_dt,b.instl_end_dt,b.tot_emi,a.dmd_amt demand_amt,g.credit collection_amt,(a.dmd_amt - g.credit) due_amt,b.outstanding curr_outstanding",
    ...
    try{
     ...
     `
       ...
       //create first date and last date
        ...
        var select = "a.branch_code,c.branch_name,SUM(b.tot_emi) total_ami,SUM(a.dmd_amt) demand_amt,SUM(d.credit) collection_amt,(SUM(a.dmd_amt) - SUM(d.credit)) due_amt,SUM(b.outstanding) curr_outstanding",
