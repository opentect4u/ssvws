  ...
  loan_overdueRouter.post("/loan_overdue_report_groupwise", async (req, res) => {
    try {
        ...
        if (!data.search_brn_id ||  data.search_brn_id.length === 0) {
           ...
           END
           ELSE 'N/A'
           END AS recovery_day,b.week_no,d.co_id code,e.emp_name co_name,SUM(a.disb_amt)loan_amt,b.instl_end_dt,SUM(b.prn_amt)outstanding_principal,SUM(b.intt_amt)outstanding_interest,SUM(a.outstanding) total_outstanding,SUM(a.od_amt) overdue,a.remarks Category`,
            ...
            order = `GROUP BY a.trf_date,a.branch_code,c.branch_name,c.area_code,b.group_code,d.group_name,d.acc_no1,d.acc_no2,f.branch_name,d.co_id,e.emp_name,b.recovery_day,b.week_no,b.disb_dt,b.instl_end_dt,b.period,b.period_mode,a.remarks
    ...
    loan_overdueRouter.post("/loan_overdue_report_fundwise", async (req, res) => {
        try {
           ...
           END
           ELSE 'N/A'
           END AS recovery_day,b.week_no,SUM(a.disb_amt)loan_amt,b.instl_end_dt,SUM(b.prn_amt) outstanding_principal,SUM(b.intt_amt) outstanding_interest,SUM(a.outstanding) total_outstanding,SUM(a.od_amt)overdue,a.remarks Category`,
          ...
          order = `GROUP BY a.trf_date,a.od_date,a.branch_code,c.branch_name,c.area_code,b.group_code,d.group_name,d.co_id,e.emp_name,b.fund_id,f.fund_name,b.recovery_day,b.week_no,b.disb_dt,b.instl_end_dt,b.period,b.period_mode,a.remarks
          ORDER BY b.group_code`;
      ...
      loan_overdueRouter.post("/loan_overdue_report_cowise", async (req, res) => {
        try {
          ...
          order = `GROUP BY a.trf_date,a.od_date,a.branch_code,c.branch_name,c.area_code,b.group_code,d.group_name,d.co_id,e.emp_name,b.recovery_day,b.instl_end_dt,b.period,b.period_mode,b.week_no,a.remarks
          ORDER BY b.group_code`;
  ...
  //loan overdue report groupwise 22.05.2025 latest
       ...
       loan_overdueRouter.post("/loan_overdue_report_branchwise", async (req, res) => {
        try {
            ...
            if (!data.search_brn_id ||  data.search_brn_id.length === 0) {
              ...
              for(let dt of data.search_brn_id){
           var select = "a.branch_code,c.branch_name,c.area_code,SUM(a.disb_amt) loan_amt,SUM(b.prn_amt) outstanding_principal,SUM(b.intt_amt) outstanding_interest,SUM(a.outstanding) total_outstanding,SUM(a.od_amt) overdue,a.remarks Category",
          table_name = "td_od_loan a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code",
          whr = `a.branch_code IN (${dt.branch_code}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${dt.branch_code})
                                                                           AND trf_date <= '${data.send_date}')`,
          order = `GROUP BY a.branch_code,c.branch_name,c.area_code,a.remarks`;
      ...
      loan_overdueRouter.post("/filter_daywise_overdue_report_groupwise", async (req, res) => {
        ...
        if (!data.search_brn_id ||  data.search_brn_id.length === 0) {
           ...
           END AS recovery_day,b.week_no,d.co_id code,e.emp_name co_name,SUM(a.disb_amt)loan_amt,b.instl_end_dt,SUM(b.prn_amt)outstanding_principal,SUM(b.intt_amt)outstanding_interest,SUM(a.outstanding) total_outstanding,SUM(a.od_amt) overdue,a.remarks Category`,
            table_name = "td_od_loan a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code LEFT JOIN md_group d ON b.group_code = d.group_code LEFT JOIN md_employee e ON d.co_id = e.emp_id",
            whr = data.period_mode == 'Fortnight' ? `a.branch_code IN (${branchCodeStr}) 
            AND b.period_mode = '${data.period_mode}' 
            AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day}
            AND b.week_no = '${data.week_no}'
            AND a.remarks = '${data.remarks}'
            AND a.trf_date = (
            SELECT MAX(trf_date) 
            FROM td_od_loan 
            WHERE branch_code IN (${branchCodeStr}) 
            AND trf_date <= '${data.send_date}')` : `a.branch_code IN (${branchCodeStr}) 
            AND b.period_mode = '${data.period_mode}' 
            AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day}
            AND a.remarks = '${data.remarks}'
            AND a.trf_date = (
            ...
            WHERE branch_code IN (${branchCodeStr}) 
            AND trf_date <= '${data.send_date}')`,
            order = `GROUP BY a.trf_date,a.branch_code,c.branch_name,c.area_code,b.group_code,d.group_name,d.acc_no1,d.acc_no2,d.branch_name,d.co_id,e.emp_name,b.recovery_day,b.week_no,b.disb_dt,b.instl_end_dt,b.period,b.period_mode,a.remarks
            ORDER BY b.group_code`;
      ...
      loan_overdueRouter.post("/filter_daywise_overdue_report_fundwise", async (req, res) => {
          ...
          whr = data.period_mode == 'Fortnight' ? `a.branch_code IN (${branchCodeStr}) 
                 ...
                 WHERE branch_code IN (${branchCodeStr})
                 AND trf_date <= '${data.send_date}') AND b.fund_id IN (${data.fund_id}) AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND b.week_no = '${data.week_no}' AND a.remarks = '${data.remarks}'` : `a.branch_code IN (${branchCodeStr}) 
                 AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                 WHERE branch_code IN (${branchCodeStr})
                 AND trf_date <= '${data.send_date}') AND b.fund_id IN (${data.fund_id}) AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND a.remarks = '${data.remarks}'`,
          order = `GROUP BY a.trf_date,a.od_date,a.branch_code,c.branch_name,c.area_code,b.group_code,d.group_name,d.co_id,e.emp_name,b.fund_id,f.fund_name,b.recovery_day,b.week_no,b.disb_dt,b.instl_end_dt,b.period,b.period_mode,a.remarks
          ORDER BY b.group_code`;
      ...
      loan_overdueRouter.post("/filter_dayawise_overdue_report_cowise", async (req, res) => {
         try {
           ...
           END AS recovery_day,b.week_no,SUM(a.disb_amt)loan_amt,b.instl_end_dt,SUM(b.prn_amt) outstanding_principal,SUM(b.intt_amt)outstanding_interest,SUM(a.outstanding) total_outstanding,SUM(a.od_amt) overdue,a.remarks Category`,
          table_name = "td_od_loan a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code LEFT JOIN md_group d ON b.group_code = d.group_code LEFT JOIN md_employee e ON d.co_id = e.emp_id",
          whr = data.period_mode == 'Fortnight' ? `a.branch_code IN (${branchCodeStr}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${branchCodeStr})
                                                                           AND trf_date <= '${data.send_date}') AND d.co_id IN (${data.co_id}) AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND b.week_no = '${data.week_no}' AND a.remarks = '${data.remarks}'` : `a.branch_code IN (${branchCodeStr}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${branchCodeStr})
                                                                           AND trf_date <= '${data.send_date}') AND d.co_id IN (${data.co_id}) AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND a.remarks = '${data.remarks}'`,
          order = `GROUP BY a.trf_date,a.od_date,a.branch_code,c.branch_name,c.area_code,b.group_code,d.group_name,d.co_id,e.emp_name,b.recovery_day,b.week_no,b.instl_end_dt,b.period,b.period_mode,a.remarks
          ORDER BY b.group_code`;
      ...
      loan_overdueRouter.post("/filter_dayawise_overdue_report_membwise", async (req, res) => {
         try {
          ...
          table_name = "td_od_loan a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code LEFT JOIN md_group d ON b.group_code = d.group_code LEFT JOIN md_employee e ON d.co_id = e.emp_id LEFT JOIN md_member f ON b.member_code = f.member_code LEFT JOIN md_scheme g ON b.scheme_id = g.scheme_id",
           whr = data.period_mode == 'Fortnight' ? `a.branch_code IN (${branchCodeStr}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${branchCodeStr})
                                                                           AND trf_date <= '${data.send_date}') AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND b.week_no = '${data.week_no}' AND a.remarks = '${data.remarks}'` : `a.branch_code IN (${branchCodeStr}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${branchCodeStr})
                                                                           AND trf_date <= '${data.send_date}') AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND a.remarks = '${data.remarks}'`,
          order = `ORDER BY a.loan_id`;
