  ...
  loan_overdueRouter.post("/loan_overdue_report_groupwise", async (req, res) => {
    ...
    loan_overdueRouter.post("/loan_overdue_report_fundwise", async (req, res) => {
        try {
           ...
           END
           ELSE 'N/A'
           END AS recovery_day,b.week_no,SUM(a.disb_amt)loan_amt,b.instl_end_dt,SUM(b.prn_amt) outstanding_principal,SUM(b.intt_amt) outstanding_interest,SUM(a.outstanding) total_outstanding,SUM(a.od_amt)overdue`,
  ...
  //loan overdue report groupwise 22.05.2025 latest
      ...
      loan_overdueRouter.post("/filter_daywise_overdue_report_fundwise", async (req, res) => {
          ...
          whr = data.period_mode == 'Fortnight' ? `a.branch_code IN (${branchCodeStr}) 
                 ...
                 WHERE branch_code IN (${branchCodeStr})
                 AND trf_date <= '${data.send_date}') AND b.fund_id IN (${data.fund_id}) AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND b.week_no = '${data.week_no}'` : `a.branch_code IN (${branchCodeStr}) 
                 AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                 WHERE branch_code IN (${branchCodeStr})
      ...
      loan_overdueRouter.post("/filter_dayawise_overdue_report_membwise", async (req, res) => {
         try {
          ...
          table_name = "td_od_loan a LEFT JOIN td_loan b ON a.loan_id = b.loan_id LEFT JOIN md_branch c ON a.branch_code = c.branch_code LEFT JOIN md_group d ON b.group_code = d.group_code LEFT JOIN md_employee e ON d.co_id = e.emp_id LEFT JOIN md_member f ON b.member_code = f.member_code LEFT JOIN md_scheme g ON b.scheme_id = g.scheme_id",
           whr = data.period_mode == 'Fortnight' ? `a.branch_code IN (${branchCodeStr}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${branchCodeStr})
                                                                           AND trf_date <= '${data.send_date}') AND b.period_mode = '${data.period_mode}' AND b.recovery_day BETWEEN ${data.from_day} AND ${data.to_day} AND b.week_no = '${data.week_no}'` : `a.branch_code IN (${branchCodeStr}) AND a.trf_date = (SELECT MAX(trf_date) FROM td_od_loan
                                                                           WHERE branch_code IN (${branchCodeStr})
