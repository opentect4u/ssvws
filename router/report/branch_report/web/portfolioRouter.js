...
portfolioRouter.post("/groupwise_portfolio_report", async (req, res) => {
    try {
        ...
        var select = `a.from_dt, a.to_dt, a.branch_cd,h.division, b.scheme_name, a.cust_type, a.group_cd, c.group_name, a.sb_ac_no,a.loan_ac_no, g.bank_name, a.applied_dt, SUM(a.applied_amt) AS applied_amt, a.disb_dt,
        ...
        table_name = "td_portfolio a LEFT JOIN md_scheme b ON a.scheme_id = b.scheme_id LEFT JOIN md_group c ON a.group_cd  = c.group_code LEFT JOIN md_fund e ON a.fund_id = e.fund_id LEFT JOIN md_purpose f ON a.purpose   = f.purp_id LEFT JOIN md_bank g ON c.bank_name = g.bank_code LEFT JOIN md_division h ON a.branch_cd = h.branch_code",
        whr = `a.branch_cd IN (${data.branch_code}) AND a.from_dt = '${data.from_dt}' AND a.to_dt = '${data.to_dt}'`,
        order =  `GROUP BY a.from_dt,a.to_dt,a.branch_cd,h.division,b.scheme_name,a.cust_type,a.group_cd,c.group_name,a.sb_ac_no,a.loan_ac_no,g.bank_name,a.applied_dt,
...
try{
    ...
    var select = "a.from_dt,a.to_dt,a.branch_cd,d.division,a.cust_type,a.group_cd,c.group_name,a.applied_dt,SUM(a.applied_amt)applied_amt,a.disb_dt,SUM(a.disb_amt)disb_amt,a.intt_rt,SUM(a.tot_emi)tot_emi,a.co_id,a.co_name,SUM(a.demand)demand,SUM(a.open_bal)open_bal,SUM(a.dr_amt)Disbursement_within_the_period,SUM(a.prn_recov) + SUM(a.intt_recov) Recovery_within_the_period,SUM(a.prn_amt)prn_amt,SUM(a.intt_amt)intt_amt,SUM(a.prn_amt) + SUM(a.intt_amt) Outstanding,SUM(a.overdue_amt)overdue_amt,a.od_dt First_overdue_date,a.od_trf_dt Overdue_transfer_date,a.loan_end_dt Loan_end_date,a.last_trn_dt Last_tranaction_date",
    table_name = "td_portfolio a LEFT JOIN md_group c ON a.group_cd  = c.group_code LEFT JOIN md_division d ON a.branch_cd = d.branch_code",
    whr = `a.branch_cd IN (${data.branch_code}) AND a.co_id IN (${data.co_id}) AND a.from_dt = '${data.from_dt}' AND a.to_dt = '${data.to_dt}'`,
    order = `GROUP BY a.from_dt,a.to_dt,a.branch_cd,d.division,a.cust_type,a.group_cd,c.group_name,a.applied_dt,
...
portfolioRouter.post("/memberwise_portfolio_report", async (req, res) => {
    try {
        ...
        var select = "a.from_dt,a.to_dt,a.branch_cd,i.division,g.form_no,a.loan_id,b.scheme_name,a.cust_type,a.group_cd,c.group_name,a.memb_id,d.client_name,d.client_mobile,d.gurd_name,d.dob,d.religion,d.caste,d.husband_name,CONCAT(d.client_addr,'-Pin:',d.pin_no) Address,d.aadhar_no,d.pan_no,d.voter_id,c.acc_no1 sb_ac_no,c.acc_no2 loan_ac_no,h.bank_name,a.applied_dt,a.applied_amt,a.disb_dt,a.disb_amt,a.proc_charge,a. service_charge,a.intt_rt,a.tot_emi,e.fund_name,f.purpose_id,a.co_id,a.co_name,a.demand,a.open_bal,a.dr_amt Disbursement_within_the_period,a.prn_recov,a.intt_recov,(a.prn_recov + a.intt_recov) Recovery_within_the_period,a.prn_amt,a.intt_amt,(a.prn_amt + a.intt_amt) Outstanding,a.overdue_amt,a.od_dt First_overdue_date,a.od_trf_dt Overdue_transfer_date,a.loan_end_dt Loan_end_date,a.last_trn_dt Last_tranaction_date",
        table_name = "td_portfolio a LEFT JOIN md_scheme b ON a.scheme_id = b.scheme_id LEFT JOIN md_group c ON a.group_cd  = c.group_code LEFT JOIN md_member d ON a.memb_id   = d.member_code LEFT JOIN md_fund e ON a.fund_id   = e.fund_id LEFT JOIN md_purpose f ON a.purpose = f.purp_id LEFT JOIN td_grt_basic g ON a.memb_id = g.member_code LEFT JOIN md_bank h ON c.bank_name = h.bank_code LEFT JOIN md_division i ON a.branch_cd = i.branch_code",
        whr = `a.branch_cd IN (${data.branch_code}) AND a.from_dt = '${data.from_dt}' AND a.to_dt = '${data.to_dt}'`,
...
portfolioRouter.post("/branchwise_portfolio_report", async (req, res) => {
    try{
        ...
        // var select = "a.from_dt,a.to_dt,a.branch_cd,d.branch_name,a.cust_type,a.group_cd,c.group_name,SUM(a.disb_amt)disb_amt,SUM(a.demand)demand,SUM(a.demand)demand,SUM(a.open_bal)open_bal,SUM(a.dr_amt)Disbursement_within_the_period,SUM(a.prn_recov) + SUM(a.intt_recov) Recovery_within_the_period,SUM(a.prn_amt)prn_amt,SUM(a.intt_amt)intt_amt,SUM(a.prn_amt) + SUM(a.intt_amt) Outstanding,SUM(a.overdue_amt)overdue_amt",
         var select = "a.from_dt,a.to_dt,a.branch_cd,d.branch_name,d.area_code,SUM(a.disb_amt)disb_amt,SUM(a.demand)demand,SUM(a.open_bal)open_bal,SUM(a.dr_amt)Disbursement_within_the_period,SUM(a.prn_recov) + SUM(a.intt_recov) Recovery_within_the_period,SUM(a.prn_amt)prn_amt,SUM(a.intt_amt)intt_amt,SUM(a.prn_amt) + SUM(a.intt_amt) Outstanding,SUM(a.overdue_amt)overdue_amt",
        table_name = "td_portfolio a LEFT JOIN md_branch d ON a.branch_cd = d.branch_code",
        whr = `a.branch_cd IN (${data.branch_code}) AND a.from_dt = '${data.from_dt}' AND a.to_dt = '${data.to_dt}'`,
        order = `GROUP BY a.from_dt,a.to_dt,a.branch_cd,d.branch_name,d.area_code`;
