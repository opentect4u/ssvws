...
portfolioRouter.post("/groupwise_portfolio_report", async (req, res) => {
    try {
        ...
        var select = `a.from_dt,a.to_dt,a.branch_cd,b.scheme_name,a.cust_type,a.group_cd,c.group_name,a.sb_ac_no,
                      ...
                      SUM(a.demand) AS demand,SUM(a.open_bal) AS open_bal,SUM(a.dr_amt) AS Disbursement within the period,SUM(a.prn_recov) AS prn_recov,SUM(a.intt_recov) AS intt_recov,SUM(a.prn_recov) + SUM(a.intt_recov) AS Recovery in the period,SUM(a.prn_amt) AS prn_amt,SUM(a.intt_amt) AS intt_amt,SUM(a.prn_amt) + SUM(a.intt_amt)) AS Outstanding,SUM(a.overdue_amt) AS overdue_amt,
