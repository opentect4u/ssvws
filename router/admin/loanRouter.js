...
loanRouter.post("/fetch_appl_dtls_via_grp", async (req, res) => {
  ...
  if (fetch_appl_dtls.suc > 0 && fetch_appl_dtls.msg.length > 0) {
    var select =
        "a.member_code,a.client_name,b.form_no,DATE_FORMAT(b.grt_date, '%Y-%m-%d') application_date,DATE_FORMAT(b.approved_at, '%Y-%m-%d') grt_approve_date,c.applied_amt,c.loan_purpose,c.sub_pupose",
      table_name =
        "md_member a JOIN td_grt_basic b ON a.branch_code = b.branch_code AND a.member_code = b.member_code LEFT JOIN td_grt_occupation_household c ON a.branch_code = c.branch_code AND b.form_no = c.form_no",
