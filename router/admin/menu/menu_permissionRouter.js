...
menu_permissionRouter.post("/menu_permission", async (req, res) => {
    try {
        ...
        if (count_user_type.user_type_no > 0) {
            var table_name = "td_menu_permission",
    fields = `
        grt = ${data.grt ? `'${data.grt}'` : 'NULL'}, 
        applications = ${data.applications ? `'${data.applications}'` : 'NULL'},
        search_member = ${data.search_member ? `'${data.search_member}'` : 'NULL'}, 
        \`groups\` = ${data.groups ? `'${data.groups}'` : 'NULL'}, 
        edit_group = ${data.edit_group ? `'${data.edit_group}'` : 'NULL'}, 
        add_group = ${data.add_group ? `'${data.add_group}'` : 'NULL'}, 
        transfer_group = ${data.transfer_group ? `'${data.transfer_group}'` : 'NULL'}, 
        approve_group_transfer = ${data.approve_group_transfer ? `'${data.approve_group_transfer}'` : 'NULL'}, 
        view_group_transfer = ${data.view_group_transfer ? `'${data.view_group_transfer}'` : 'NULL'}, 
        attendance =  ${data.attendance ? `'${data.attendance}'` : 'NULL'}, 
        attendance_dashboard =  ${data.attendance_dashboard ? `'${data.attendance_dashboard}'` : 'NULL'}, 
        loans = ${data.loans ? `'${data.loans}'` : 'NULL'}, 
        disburse_loan = ${data.disburse_loan ? `'${data.disburse_loan}'` : 'NULL'}, 
        view_loan = ${data.view_loan ? `'${data.view_loan}'` : 'NULL'}, 
        approve_transaction = ${data.approve_transaction ? `'${data.approve_transaction}'` : 'NULL'},
        reports = ${data.reports ? `'${data.reports}'` : 'NULL'},
        loan_statement = ${data.loan_statement ? `'${data.loan_statement}'` : 'NULL'}, 
        loan_transactions = ${data.loan_transactions ? `'${data.loan_transactions}'` : 'NULL'}, 
        demand_report = ${data.demand_report ? `'${data.demand_report}'` : 'NULL'},
        outstanding_report = ${data.outstanding_report ? `'${data.outstanding_report}'` : 'NULL'}, 
        fundwise_report = ${data.fundwise_report ? `'${data.fundwise_report}'` : 'NULL'}, 
        schemewise_report = ${data.schemewise_report ? `'${data.schemewise_report}'` : 'NULL'}, 
        demand_vs_collection = ${data.demand_vs_collection ? `'${data.demand_vs_collection}'` : 'NULL'}, 
        master = ${data.master ? `'${data.master}'` : 'NULL'}, 
        banks = ${data.banks ? `'${data.banks}'` : 'NULL'},
        employees = ${data.employees ? `'${data.employees}'` : 'NULL'}, 
        designation = ${data.designation ? `'${data.designation}'` : 'NULL'}, 
        user_management = ${data.user_management ? `'${data.user_management}'` : 'NULL'}, 
        create_user = ${data.create_user ? `'${data.create_user}'` : 'NULL'}, 
        manage_user = ${data.manage_user ? `'${data.manage_user}'` : 'NULL'}, 
        transfer_user = ${data.transfer_user ? `'${data.transfer_user}'` : 'NULL'}, 
    ...
    var menu_permission_data = await db_Insert(table_name, fields, values, null, flag);
        ...
        res.send({ "suc": 1, "msg": "Menu permission updated successfully"});
    } catch (error) {
        console.error(error);
        res.send({ "suc": 0, "msg": "Error updating menu permission"});
    }
});
