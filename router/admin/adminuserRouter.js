...
adminuserRouter.post("/save_bank_dtls", async (req, res) => {
    ...
    fields = data.bank_code > 0 ? `bank_name = '${data.bank_name}', branch_name = '${data.branch_name}', ifsc = '${data.ifsc == '' ? 0 : data.ifsc}', branch_addr = '${data.branch_addr.split("'").join("\\'")}', sol_id = '${data.sol_id == '' ? 0 : data.sol_id}', phone_no = '${data.phone_no > 0 ? data.phone_no : 0}', dist_code = '${data.dist_code == '' ? 0 : data.dist_code}', modified_by = '${data.modified_by}', modified_at = '${datetime}'` : `(bank_code, bank_name, branch_name, ifsc, branch_addr, sol_id, phone_no, dist_code, created_by, created_at)`,
