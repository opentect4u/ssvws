                    ...
                    if (fetch_member.suc > 0 && fetch_member.msg.length > 0) {
                        for (let member of fetch_member.msg) {
                            ...
                            var table_name = "td_grt_basic_rep",
                                ...
                                values = `('${member.form_no}',${member.grt_date ? `'${member.grt_date}'` : 'NULL'},'${member.branch_code}','${member.prov_grp_code}','${member.member_code}','${member.approval_status}',${member.remarks ?  `'${member.remarks}'` : 'NULL'},'${member.grp_added_by}',${member.grp_added_at ? `'${member.grp_added_at}'` : 'NULL'},'${member.delete_flag}','${member.deleted_by}',${member.deleted_at ? `'${member.deleted_at}'` : 'NULL'},'${member.created_by}',${member.created_at ? `'${member.created_at}'` : 'NULL'},'${member.modified_by}',${member.modified_at ? `'${member.modified_at}'` : 'NULL'},'${member.approved_by}',${member.approved_at ? `'${member.approved_at}'` : 'NULL'},'${member.rejected_by}',${member.rejected_at ? `'${member.rejected_at}'` : 'NULL'},'${member.co_lat_val}','${member.co_long_val}','${member.co_gps_address}','${member.bm_lat_val}','${member.bm_long_val}','${member.bm_gps_address}')`,
                                whr = null,
                                flag = 0;
