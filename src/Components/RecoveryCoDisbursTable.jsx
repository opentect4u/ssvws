...
function RecoveryCoDisbursTable({
	loanAppData,
	setSearch_Co,
	title,
	flag,
	...
	const fetchLoanGroupMember = async (payment_date) => {
		setLoading(true)
		await axios
			.post(`${url}/fetch_cowise_disb_member_dtls`, {
	...
	const rejectRecoveryTransaction = async (cachedPaymentId) => {
		...
		const creds = {
			...
			reject_dt_disb: cachedPaymentId,
		}
		console.log(creds, "rejectRecoveryTransaction")
			...
			<DialogBox
				...
				onPressYes={async () => {
					...
					await approveRecoveryTransaction(cachedPaymentId)
						...
						.catch((err) => {
							console.log("Err in RecoveryCoDisbursTable.jsx", err)
						})
					setVisible(!visible)
				}}
				onPressNo={() => {
...
export default RecoveryCoDisbursTable
