    ...
    loan_trans: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            if(trans_dt.suc > 0 && trans_dt.msg.length > 0){
                ...
                fields = data.loan_code > 0 ? `payment_date = '${datetime}', particulars = '${data.particulars.split("'").join("\\'")}', credit = '0', debit = '${data.prn_disb_amt > 0  ? data.prn_disb_amt : 0}', bank_charge = '${data.bank_charge > 0 ? data.bank_charge : 0}', proc_charge = '${data.proc_charge > 0 ? data.proc_charge : 0}', prn_recov = '0', intt_recov = '0', balance = '${data.prn_disb_amt > 0 ? data.prn_disb_amt : 0}', tr_type = '${data.tr_type}', tr_mode = '${data.tr_mode}', cheque_id = '${data.cheque_id == '' ? 0 : data.cheque_id}', chq_dt = '${data.chq_dt == '' ? null : data.chq_dt}', modified_by = '${data.created_by}', modified_at = '${datetime}'` : `(payment_date,payment_id,branch_id,loan_id,particulars,credit,debit,bank_charge,proc_charge,prn_recov,intt_recov,balance,tr_type,tr_mode,cheque_id,chq_dt,status,created_by,created_at)`,
                values = `('${datetime}', '${payment_id}', '${data.branch_code == '' ? 0 : data.branch_code}', '${loan_code}', '${data.particulars.split("'").join("\\'")}', '0', '${data.prn_disb_amt > 0 ?  data.prn_disb_amt : 0}', '${data.bank_charge > 0 ? data.bank_charge : 0}', '${data.proc_charge > 0 ? data.proc_charge : 0}', '0', '0', '${data.prn_disb_amt > 0 ? data.prn_disb_amt : 0}', '${data.tr_type}', '${data.tr_mode}', '${data.cheque_id == '' ? 0 : data.cheque_id}', '${data.chq_dt == '' ? null : data.chq_dt}', 'U', '${data.created_by}', '${datetime}')`,
                whr = data.loan_code > 0 ? `loan_id = ${data.loan_code}` : null,
                flag = data.loan_code > 0 ? 1 : 0;
