    ...
    loan_trans: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            if(trans_dt.suc > 0 && trans_dt.msg.length > 0){
                ...
                values = `('${datetime}', '${payment_id}', '${data.branch_code == '' ? 0 : data.branch_code}', '${loan_code}', '${data.particulars.split("'").join("\\'")}', '0', '${dts.prn_disb_amt > 0 ?  dts.prn_disb_amt : 0}', '0', '${data.proc_charge > 0 ? data.proc_charge : 0}', '0', '0', '${dts.prn_disb_amt > 0 ? dts.prn_disb_amt : 0}', '0', '${intt_cal_amt}', 'D', 'B', '${data.bank_name}', '0', 'U', '${data.created_by}', '${datetime}')`,
                whr = null,
                flag = 0;
                var dtls = await db_Insert(table_name,fields,values,whr,flag);
            }
