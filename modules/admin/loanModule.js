               ...
               try{
                ...
                if (data.disbdtls.length > 0) {
                    for (let dts of data.disbdtls) {
                        var selectedMode = periodic.filter(
                            (dt) => dt.id == data.period_mode
                        );

                        var inst_end_date = new Date(
                            curr_date.getFullYear(),
                            curr_date.getMonth() + 1,
                            0
                        );
                        switch (data.period_mode) {
                            case "Monthly":
                                inst_end_date.setMonth(
                                    last_date.getMonth() +
                                    data.period / selectedMode[0].div_period
                                );
                                break;
                            case "Weekly":
                                inst_end_date.setDate(
                                    last_date.getDay() +
                                    (data.period / selectedMode[0].div_period) * 7
                                );
                                break;
                        ...
                        let loan_code = await getLoanCode(data.branch_code);
                        let intt_cal_amt = await interest_cal_amt(
                            dts.prn_disb_amt,
                            data.period,
                            data.curr_roi,
                            data.period_mode
                        ...
                        let outstanding =
                            parseFloat(dts.prn_disb_amt) +
                            parseFloat(data.old_prn_amt) +
                            parseFloat(intt_cal_amt) +
                            parseFloat(data.od_intt_amt);
                        // console.log(outstanding,'outstanding');
                        ...
                        if (trans_dt.suc > 0 && trans_dt.msg.length > 0) {
                            ...
                            var table_name = "td_loan_transactions",
                                ...
                                values = `('${datetime}', '${payment_id}', '${data.branch_code == "" ? 0 : data.branch_code
                                    ...
                                    }', '${data.bank_charge > 0 ? data.bank_charge : 0}', '${data.proc_charge > 0 ? data.proc_charge : 0
                                    }', '0', '0', '${dts.prn_disb_amt > 0 ? dts.prn_disb_amt : 0
                                    }', '0', '${intt_cal_amt}', 'D', 'B', '${data.bank_name
                                    }', '0', 'U', '${data.created_by}', '${datetime}')`,
                                whr = null,
                                flag = 0;
                            var dtls = await db_Insert(table_name, fields, values, whr, flag);
                        }
                ...
                reject({ suc: 0, msg: "Transaction failed" });
            }
        });
    },
