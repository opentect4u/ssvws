    ...
    emp_details_save: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            var table_name = "md_employee",
                ...
                values = `('${emp_code}','${data.branch_code}','0','${data.emp_name}','${data.gender}','${data.guardian_name}','${data.addr.split("'").join("\\'")}','${data.district.split("'").join("\\'")}','${data.pin_code == '' ? 0 : data.pin_code}','${data.phone_home == '' ? 0 : data.phone_home}','${data.phone_mobile == '' ? 0 : data.phone_mobile}',${data.email == '' ? 'NULL' : `'${data.email}'`},'${data.designation == '' ? 0 : data.designation}',${data.nationality == '' ? 'NULL' : `'${data.nationality}'`},${data.dob == '' ? 'NULL' : `'${data.dob}'`},${data.married == '' ? 'NULL' : `'${data.married}'`},${data.language_known == '' ? 'NULL' : `'${data.language_known}'`},${data.doj == '' ? 'NULL' : `'${data.doj}'`},'${data.prob_period == '' ? 0 : data.prob_period}','${data.retairment_age == '' ? 0 : data.retairment_age}',${data.conf_dt == '' ? 'NULL' : `'${data.conf_dt}'`},${data.retair_dt == '' ? 'NULL' : `'${data.retair_dt}'`},${data.blood_grp == '' ? 'NULL' : `'${data.blood_grp}'`}, ${data.voter_id == '' ? 'NULL' : `'${data.voter_id}'`},${data.pan_no == '' ? 'NULL' : `'${data.pan_no}'`},${data.aadhar_no == '' ? 'NULL' : `'${data.aadhar_no}'`},${data.bank_name == '' ? 'NULL' : `'${data.bank_name}'`},${data.branch_name == '' ? 'NULL' : `'${data.branch_name}'`},${data.ifsc == '' ? 'NULL' : `'${data.ifsc}'`},${data.acc_no == '' ? 'NULL' : `'${data.acc_no}'`},'${data.remarks.split("'").join("\\'")}','${data.created_by}','${datetime}')`,
