    ...
    edit_grp_web: (data) => {
        return new Promise(async (resolve, reject) => {
          try{
            ...
            '${data.email_id}', '${data.grp_addr.split("'").join("\\'")}', '${data.district}', '${data.block ? data.block : 0}', '${data.pin_no}', ${data.bank_name == '' ? 'NULL' : data.bank_name}, ${data.branch_name == '' ? 'NULL' : data.branch_name}, '${data.ifsc == '' ? 0 : data.ifsc}', '${data.micr == '' ? 0 : data.micr}', '${data.acc_no1 == '' ? 0 : data.acc_no1}',
            ...
            if(edit_grp_dtls.suc > 0 && edit_grp_dtls.msg.length > 0){
              if (data.grp_memberdtls.length > 0) {
                for (let dt of data.grp_memberdtls) {
                  ...
                  fields = `prov_grp_code = '${prov_grp_code == 0 ? data.group_code : group_code}', modified_by = '${data.modified_by}', modified_at = '${datetime}'`,
                  values = null,
