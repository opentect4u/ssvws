...
module.exports = {
    ...
    edit_household_dt: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            if(res_dt.msg.length > 0 && res_dt.suc > 0){
                var table_name = "td_grt_occupation_household",
                    fields = `no_of_rooms = '0', house_type = ${data.house_type == '' ? 'NULL' : data.house_type}, own_rent = ${data.own_rent == '' ? 'NULL' : data.own_rent}, land = '${data.land == '' ? 0 : data.land}', tv_flag = ${data.tv_flag == '' ? 'NULL' : data.tv_flag}, 
                    bike_flag = ${data.bike_flag == '' ? 'NULL' : data.bike_flag}, fridge_flag = ${data.fridge_flag == '' ? 'NULL' : data.fridge_flag}, wm_flag = ${data.wm_flag == '' ? 'NULL' : data.wm_flag}, poltical_flag = ${data.poltical_flag == '' ? 'NULL' : data.political_flag},
                ...
                var table_name = "td_grt_occupation_household",
                    ...
                    values = `('0', ${data.house_type == '' ? 'NULL' : data.house_type}, ${data.own_rent == '' ? 'NULL' : data.own_rent}, '${data.land == '' ? 0 : data.land}', ${data.tv_flag == '' ? 'NULL' : data.tv_flag}, ${data.bike_flag == '' ? 'NULL' : data.bike_flag}, ${data.fridge_flag == '' ? 'NULL' : data.fridge_flag}, ${data.wm_flag == '' ? 'NULL' : data.wm_flag},${data.poltical_flag == '' ? 'NULL' : data.political_flag}, '${data.parental_addr.split("'").join("\\'")}', '${data.parental_phone == '' ? 0 : data.parental_phone}', '${data.created_by}', ${datetime})`,
                    whr = null,
                    flag = 0;
                    var house_hold_dt = await db_Insert(table_name, fields, values, whr, flag);
            }
