        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(res_dt.msg.length > 0 && res_dt.suc > 0){
                ...
                var table_name = "td_grt_occupation_household",
                    ...
                    values = `('0', '${data.house_type}', '${data.own_rent}', '${data.land == '' ? 0 : data.land}', '${data.tv_flag}', '${data.bike_flag}', '${data.fridge_flag}', '${data.wm_flag}','${data.poltical_flag}', '${data.parental_addr.split("'").join("\\'")}', '${data.parental_phone == '' ? 0 : data.parental_phone}', '${data.created_by}', ${datetime})`,
                    whr = null,
                    flag = 0;
                    var house_hold_dt = await db_Insert(table_name, fields, values, whr, flag);
            }
