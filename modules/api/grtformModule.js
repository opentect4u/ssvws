    ...
    save_basic_dtls: (data) => {
        return new Promise(async (resolve, reject) => {
            ...
            if(mobile_dt.suc > 0 && mobile_dt.msg.length == 0){
                ...
                values =  `('${data.branch_code}', '${member_code.msg[0].memberCode}', '${data.gender}', '${data.client_name}', '${datetime}',
                ...
                values =  `('${datetime}', '${data.branch_code}', '${data.prov_grp_code == '' ? 0 : data.prov_grp_code}', '${member_code.msg[0].memberCode}', 'U', '${data.co_lat_val}', '${data.co_long_val}', '${data.co_gps_address}', '${data.created_by}', '${datetime}')`,
                whr = null,
                flag = 0;
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if(res_dt.msg.length > 0 && res_dt.suc > 0){
                ...
                var table_name = "td_grt_occupation_household",
                    ...
                    values = `('${data.no_of_rooms}', '${data.house_type}', '${data.own_rent}', '${data.land}', '${data.tv_flag}', '${data.bike_flag}', '${data.fridge_flag}', '${data.wm_flag}','${data.poltical_flag}', '${data.parental_addr}', '${data.parental_phone}', '${data.created_by}', ${datetime})`,
                    whr = null,
                    flag = 0;
                    var house_hold_dt = await db_Insert(table_name, fields, values, whr, flag);
            }
            ...
            if (data.memberdtls.length > 0) {
                ...
                for (let dt of data.memberdtls) {
                    if (dt.sl_no && dt.sl_no > 0) { 
                        var table_name = "td_grt_family",
                            fields = `grt_date = '${datetime}', family_name = '${dt.name}', relation = '${dt.relation}', age = '${dt.age}', sex = '${dt.sex}', education = '${dt.education}', stu_work_flag = '${dt.studyingOrWorking}', monthly_income = '${dt.monthlyIncome}', modified_by = '${data.modified_by}', modified_at = '${datetime}'`,
                            values = null,
