...
module.exports = {
  ...
  recovery_trans: (data) => {
    ...
    return new Promise(async (resolve, reject) => {
      let datetime = dateFormat(new Date(), "yyyy-mm-dd HH:MM:ss");
      var trans_dtl = { suc: 1, msg: "" };
      var not_inserted_row = [];
      try {
        let datevalidation = await fetch_date(
          data.branch_code,
        ...
        if (datevalidation.suc > 0) {
          if (data.recovdtls.length > 0) {
            ...
            for (let dt of data.recovdtls) {
              ...
              if (isNaN(prnEmi) || isNaN(inttEMI) || isNaN(creditAmt)) {
                return reject({
                  suc: 0,
                  msg: "Invalid recovery amount provided.",
                });
              }

              if (
                Number(prnEmi + inttEMI).toFixed(2) !== creditAmt.toFixed(2)
              ) {
                return reject({
                  suc: 0,
            ...
            for (let dt of data.recovdtls) {
              ...
              try {
                if (dt.credit > 0 && dt.credit) {
                  ...
                  intt_balance = !isNaN(intt_balance) ? intt_balance : 0;
                  inttEMI = !isNaN(inttEMI) && inttEMI > 0 ? inttEMI : 0;
                    ...
                    if (rec_dtls_prn.suc > 0 && rec_dtls_prn.msg.length > 0) {
                      ...
                      var table_name = "td_loan",
                        fields = `prn_amt = '${
                          ...
                          dt.instl_paid > 0 ? dt.instl_paid : 0
                        }', last_trn_dt = '${dt.last_trn_dt}', modified_by = '${
                          data.modified_by
                      ...
                      if (rec_dt.suc > 0 && rec_dt.msg.length > 0) {
                        var select = `a.loan_id,a.member_code,a.branch_code,a.group_code,a.outstanding memb_outstanding,b.payment_date tnx_date,b.tr_mode,b.cheque_id,b.credit,b.created_by collec_code,b.upload_on,c.group_name,d.branch_name,e.emp_name collec_name,f.client_name,(
                          ...
                          whr = `a.group_code = '${data.group_code}' AND b.tr_type = 'R' AND b.created_at = 
                                      (SELECT MAX(created_at) 
                                                          FROM td_loan_transactions 
                                                      WHERE loan_id IN (${dt.loan_id}))`,
                          order = `GROUP BY a.loan_id,a.member_code,a.branch_code,a.group_code,a.outstanding,b.payment_date,b.credit,b.tr_mode,b.cheque_id,b.created_by,b.upload_on,c.group_name,d.branch_name,e.emp_name,f.client_name`;
                        trans_dtl = await db_Select(
                          select,
                          table_name,
                          whr,
                          order
                        ...
                        reject({
                          suc: 0,
                          msg: "Print receipt data not generated",
                        });
                      }
                    }
          ...
          reject({
            suc: 0,
            msg: "Transaction date must be greater than closing date",
          });
        }
      } catch (error) {
        console.error("Error in recovery_trans:", error);
