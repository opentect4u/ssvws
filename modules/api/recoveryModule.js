            ...
            if(data.recovdtls.length > 0){
                ...
                let payment_id = await payment_code(data.branch_code)     //interest
                    ...
                    if(rec_dtls_int.suc > 0 && rec_dtls_int.msg.length > 0){
                        ...
                        values = `('${datetime}','${payment_id}','${data.branch_code == '' ? 0 : data.branch_code}','${dt.loan_id}','${data.tr_mode == 'U' ? 'By UPI [Installment Payment]' : data.tr_mode == 'C' ? 'By Cash [Installment Payment]' : ''}','${dt.credit}','0','${prnEmi > 0 ?prnEmi : 0}','${inttEMI > 0 ? inttEMI : 0}','${prn_recov > 0 ? prn_recov : 0}','0','${datetime}','R','${data.tr_mode}','${data.bank_name}','${data.cheque_id == '' ? 0 : data.cheque_id}', '${data.chq_dt == '' ? null : data.chq_dt}','U','${data.created_by}','${datetime}','${data.trn_lat}','${data.trn_long}','${data.trn_addr.split("'").join("\\'")}')`,
                        whr = null,
                        flag = 0;
                        var rec_dtls_prn = await db_Insert(table_name,fields,values,whr,flag);    
                    }
